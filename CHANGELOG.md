...
- **Kafka Scaler**: Add optional `ccacheName` authentication parameter to support SASL/GSSAPI authentication with an existing Kerberos credential cache, resolved by file name against the operator's `/tmp/kerberos/ccache/` directory ([#7845](https://github.com/kedacore/keda/issues/7845))
